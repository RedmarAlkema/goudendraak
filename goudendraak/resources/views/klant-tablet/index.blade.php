...
<body>
    <div id="app" class="container mt-4">
        @if(Session::has('round'))
            <div class="alert alert-secondary position-fixed top-0 start-0 mt-2 ms-2">
        ...
        @if(Session::has('table'))
            <div class="text-center">
                <h2>Tafelnummer: {{ Session::get('table')->table_number }}</h2>
            </div>
        @else
            <div class="card mx-auto mt-4" style="max-width: 400px;">
                <div class="card-body text-center">
                    <form action="{{ route('cart.storeTableNumber') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="table_number" class="form-label">Voer uw tafelnummer in:</label>
                            <input type="number" id="table_number" name="table_number" class="form-control" min="1" required>
                        </div>
                        <button type="submit" class="btn btn-primary">Tafelnummer opslaan</button>
                    </form>
                </div>
            </div>
        @endif
